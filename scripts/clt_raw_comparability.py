...
def _fit_pca_bundles(
    *,
...
) -> Tuple[Dict[int, LayerPCABundle], Dict[int, Dict[str, LayerPCABundle]], Dict[int, Dict[str, int]]]:
    def _build_bundle(chunks: Sequence[torch.Tensor], *, n_pairs_fit: int) -> Optional[LayerPCABundle]:
        if not chunks:
            return None
        ...
        components = vh.to(device="cpu", dtype=torch.float32)
        return LayerPCABundle(
    ...
    fit_meta: Dict[int, Dict[str, int]] = {}
    for layer in layers:
        l = int(layer)
        ...
        if not pair_chunks:
            ...
            continue
        global_bundle = _build_bundle(list(pair_chunks.values()), n_pairs_fit=len(pair_chunks))
        ...
        for excluded_pair in sorted(pair_chunks.keys()):
            ...
            loo_bundle = _build_bundle(train_chunks, n_pairs_fit=max(0, len(pair_chunks) - 1))
