...
from aom.data.loaders import load_coherence_items_with_manifest
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="AoM-COH causal patching runner (constraint-ablation protocol).")
    p.add_argument(
        "--config",
        type=str,
        default="",
        help=(
            ...
            "Relative paths inside configs are resolved relative to the repo root (directory containing aom_coh_patching.py)."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=20,
        help="Skip items where abs(len(main_ids)-len(ablate_ids)) exceeds this (default: 20).",
    )
...
def main() -> None:
    ...
    if not csv_path:
        ...
        run_name = str(getattr(args, "run_name", "") or "").strip() or "coh_patching"
    ...
    try:
        ...
        from aom.interventions.patching.coh_protocol import COHConstraintAblationProtocol, COHPatchingConfig
        ...
        invalid = int(datasets["coh"].get("n_rows_invalid", 0) or 0)
        total = int(datasets["coh"].get("n_rows_total", 0) or 0)
        valid = int(datasets["coh"].get("n_rows_valid", 0) or 0)
        if total > 0 and valid == 0:
            raise ValueError(f"Dataset coh has {invalid}/{total} invalid rows and 0 valid rows")
        if invalid > 0:
            msg = f"dataset coh: invalid_rows {invalid} of {total}"
        ...
        protocol = COHConstraintAblationProtocol(
            config=COHPatchingConfig(max_total_len_delta=int(getattr(args, "max_total_len_delta", 20)))
        )
        ...
        for model_name in models:
            ...
            for s in seeds:
                ...
                row: Dict[str, Any] = {
                    ...
                    "coh_sha256": _sha256_file(Path(str(getattr(args, "coh_path", "")))),
                    ...
                    "max_total_len_delta": int(getattr(args, "max_total_len_delta", 20)),
                }
