...
def test_redact_argv_redacts_system_prompt_value():
    argv = ["python", "aom_eval.py", "--system_prompt", "SECRET", "--x", "y"]
    redacted = redact_argv(argv)
    assert "SECRET" not in redacted
    assert redacted[0:4] == ["python", "aom_eval.py", "--system_prompt", "<redacted>"]


def test_redact_argv_redacts_equals_form():
    argv = ["python", "aom_eval.py", "--system_prompt=SECRET"]
    redacted = redact_argv(argv)
    assert redacted == ["python", "aom_eval.py", "--system_prompt=<redacted>"]


def test_redact_argv_redacts_system_prompt_file():
    argv = ["python", "aom_eval.py", "--system_prompt_file", "/tmp/secret.txt"]
    redacted = redact_argv(argv)
    assert "/tmp/secret.txt" not in redacted
    assert redacted[0:4] == ["python", "aom_eval.py", "--system_prompt_file", "<redacted>"]


def test_redact_argv_redacts_system_prompt_file_equals_form():
    argv = ["python", "aom_eval.py", "--system_prompt_file=/tmp/secret.txt"]
    redacted = redact_argv(argv)
    assert redacted == ["python", "aom_eval.py", "--system_prompt_file=<redacted>"]


def test_run_manifest_roundtrip(tmp_path: Path):
    argv = ["python", "aom_eval.py", "--system_prompt", "SECRET", "--seed", "0"]
