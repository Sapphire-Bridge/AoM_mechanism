...
from .cue_leakage import cue_stats_for_disamb_pair
...
def compute_aom_disamb(
    ...
    *,
    ...
    cue_stats = [cue_stats_for_disamb_pair(it) for it in items]
    overlap_median = _median([float(s.pair_jaccard_mean) for s in cue_stats])

    pair_acc_samples: List[float] = []
    pair_margin_samples: List[float] = []

    acc_cue_balanced: List[float] = []
    acc_cue_vulnerable: List[float] = []
    acc_overlap_low: List[float] = []
    acc_overlap_high: List[float] = []

    margin_cue_balanced: List[float] = []
    margin_cue_vulnerable: List[float] = []
    margin_overlap_low: List[float] = []
    margin_overlap_high: List[float] = []

    for it, s in zip(items, cue_stats):
        ...
        if bool(s.asymmetric_flag_ge2):
            acc_cue_vulnerable.append(pair_acc)
            margin_cue_vulnerable.append(pair_margin)
        else:
            acc_cue_balanced.append(pair_acc)
            margin_cue_balanced.append(pair_margin)

        if float(s.pair_jaccard_mean) <= float(overlap_median):
            acc_overlap_low.append(pair_acc)
            margin_overlap_low.append(pair_margin)
        else:
            acc_overlap_high.append(pair_acc)
            margin_overlap_high.append(pair_margin)
    ...
    out: Dict[str, Any] = {
        ...
        **_boot("mean_margin", pair_margin_samples),
        ...
        # Cue leakage stratification (surface overlap diagnostic).
        "cue_overlap_median": float(overlap_median),
        "n_pairs_cue_balanced": int(len(acc_cue_balanced)),
        "n_pairs_cue_vulnerable": int(len(acc_cue_vulnerable)),
        "n_pairs_overlap_low": int(len(acc_overlap_low)),
        "n_pairs_overlap_high": int(len(acc_overlap_high)),
        **_boot("accuracy_cue_balanced", acc_cue_balanced),
        **_boot("accuracy_cue_vulnerable", acc_cue_vulnerable),
        **_boot("accuracy_overlap_low", acc_overlap_low),
        **_boot("accuracy_overlap_high", acc_overlap_high),
        **_boot("mean_margin_cue_balanced", margin_cue_balanced),
        **_boot("mean_margin_cue_vulnerable", margin_cue_vulnerable),
        **_boot("mean_margin_overlap_low", margin_overlap_low),
        **_boot("mean_margin_overlap_high", margin_overlap_high),
    }
    return out
